# SMABL44 0 daily XX-11060 QUBT-TSLA-0000-0000
...
class TradingStrategy(Strategy):
    def __init__(self):
        self.dur = 11060;self.debug = 0;self.adj_ma = 1; self.adj_bollinger=0.01*0  #self.adj_ma = 1; self.adj_bollinger=0.04
        self.tickers = ["QUBT","TSLA","BIL"]
    ...
    def run(self, data):
        ...
        for ticker in self.tickers:
            ...
            if 1==2:
              ...
              # good for longer range like 150
              elif 31001 <= self.dur <= 31999:
                df_since_start = df[-max(max(7,int(locdur/2)),min(self.count,locdur)):]
              elif 32001 <= self.dur <= 32999:
                df_since_start = df[-oscill(max(7,int(locdur/2)),locdur,self.count):]
              elif 33001 <= self.dur <= 33999:
                df_since_start = df[-oscill63(max(7,int(locdur/2)),locdur,self.count):]

              else:
                df_since_start = df[-self.locdur:]
